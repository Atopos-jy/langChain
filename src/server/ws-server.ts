...
import { strategyLabel } from "./memory-strategies";
...
export function createServer(): WebSocketServer {
    ...
    logger.info(`📝 记忆策略: /memory buffer | window | summary | vector （默认 buffer）`);
    ...
    wss.on("connection", (ws) => {
        ...
        ws.send(JSON.stringify({
            type: "welcome",
            content: "欢迎连接到 AI 对话服务器！（输入 /help 查看命令）",
            sessionId,
        }));
...
function handleCommand(ws: WebSocket, sessionId: string, cmd: string, session: Session): boolean {
    ...
    switch (name) {
        case "memory": {
            const validStrategies = ["buffer", "window", "summary", "vector"];
            const key = arg.trim().toLowerCase();
            if (!validStrategies.includes(key)) {
                ws.send(JSON.stringify({
                    type: "error",
                    content: `未知记忆策略 "${arg}"，可选: ${validStrategies.join(", ")}`,
                }));
                return true;
            }
            session.memoryStrategy = key as Session["memoryStrategy"];
            // 向量策略：恢复时注入系统提示
            ws.send(JSON.stringify({
                type: "system_set",
                content: `✅ 已切换为 ${strategyLabel(key)}`,
            }));
            logger.info(`🧠 [${sessionId}] 记忆策略已切换: ${key}`);
            return true;
        }
        ...
        case "help": {
            ...
            ws.send(JSON.stringify({
                type: "system_set",
                content: `📖 可用命令:\n`
                    + `🧠 /memory <buffer|window|summary|vector> — 切换记忆策略\n`
                    + `📝 /template <名称> — 切换提示词模板\n`
                    + `  ${templates}\n`
                    + `🗑️ /clear — 清除对话历史\n`
                    + `❓ /help — 显示此帮助`,
